...
from cython_compile.cython_compile import py2pyx, py2pyx_autodeclare
import inspect
import numpy as np
import unittest
...
class Test_Py2Pyx(unittest.TestCase):
    ...
    def test_autodeclare_all_types(self):
        def temp(a, b, c, d, e):
            pass
        self.assertEqual(self.pyx_header + ['        cpdef temp(long a, double b, long long c, np.ndarray[np.int32_t,ndim=1] d, np.ndarray[np.float64_t,ndim=2] e):\n',
                                            '            pass\n'],
                         py2pyx_autodeclare(temp, (1, 2., 3L, np.array([4]), np.array([[5], [6]], dtype=np.float64)), {}))
